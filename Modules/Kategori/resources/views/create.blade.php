    ...
    <div class="max-w-2xl mx-auto px-4">
        ...
        <div class="bg-white rounded-3xl shadow-lg shadow-blue-50 border border-slate-100 p-8 md:p-10">
            <div class="text-center mb-8">
                <h1 class="text-2xl font-extrabold text-slate-800 mb-2">Tambah Kategori Baru</h1>
                <p class="text-slate-500 text-sm">Masukkan detail jenis pungli baru.</p>
            </div>

            <form action="/kategoris" method="POST" class="space-y-6">
                @csrf

                <div>
                    <x-input-label value="Nama Kategori" />
                    <x-text-input type="text" name="nama_kategori" placeholder="Contoh: Pungli Parkir" required class="px-4 py-3" />
                </div>

                <div>
                    <x-input-label value="Deskripsi (Opsional)" />
                    <x-textarea name="deskripsi" rows="3" placeholder="Jelaskan singkat tentang kategori ini..." class="px-4 py-3"></x-textarea>
                </div>
                ...
                <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-blue-200 transition transform active:scale-95 flex items-center justify-center gap-2">
                    <i class='bx bx-save text-xl'></i> Simpan Kategori
                </button>
            </form>
        </div>
    </div>
